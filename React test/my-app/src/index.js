...
class Board {
  ...
  getMaxSequences() {
    ...
    count +=
      2 *
      (this.cols - this.winSequence + 1) *
      (this.rows - this.winSequence + 1); //diagnoals
  ...
  getNeighbors(row, col) {
    ...
    console.assert(
      col >= 0 && row >= 0 && col <= this.cols && row <= this.rows
    );
...
class ReactGame extends React.Component {
    ...
    switch (actionType) {
      ...
      case PLAY_COMPUTER:
        ...
        if (computerMove != null) {
          ...
          this.state.squares[row][computerMove] = this.computerWhite
            ? WHITE_TOKEN
            : BLACK_TOKEN;
        }
        break;
  ...
  render() {
    ...
    if (this.game.outcome === WHITE_WIN || this.game.outcome === BLACK_WIN) {
      let winningToken =
        this.game.outcome === WHITE_WIN ? WHITE_TOKEN : BLACK_TOKEN;
